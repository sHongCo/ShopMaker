...
<%
	...
	for (int i = 0; i < oProducts.length; i++) {
		...
		// 장바구니에서 삭제
		dao2.deleteWishList(dnums[i]);

		//재고 빼기
		PVO pvo = dao2.getProduct(ovo.getoProduct());
		int res_Quan;
		res_Quan = pvo.getpQuan() - ovo.getoQuan();
		pvo.setpQuan(res_Quan);

		dao2.modifyProduct(pvo);
	}
	System.out.println("ordersPro ovo.getoNum() : " + ovo.getoNum());
	System.out.println("ordersPro oNum : " + oNum);
	System.out.println("ordersPro oProduct " + request.getParameterValues("oProduct"));
...
<meta http-equiv="Refresh"
	content="0;url=result.jsp?oNum=<%=ovo.getoNum()%>&oPrice=<%=oPrice%>">
